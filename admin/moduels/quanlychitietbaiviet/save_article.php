...
if (isset($_POST['themCTbaiviet'])) {
    $iddanhmuc = $mysqli->real_escape_string($_POST['iddanhmuc']);
...
if(isset($_FILES['anh']) && $_FILES['anh']['error'] == 0) {
    ...
    $sql = "INSERT INTO danhmucc (iddanhmuc, title, hinhanh,mota) VALUES ('$iddanhmuc', '$title', '$hinhanh','$mota')";
    if ($mysqli->query($sql)) {
        ...
        if (!empty($_POST['blocks'])) {
            foreach ($_POST['blocks'] as $block) {
                ...
                $sql = "INSERT INTO chitietdanhmuc
                        (iddanhmuc,article_id, block_type, content, image_url, caption, sort_order) 
                        VALUES 
                        ('$iddanhmuc','$article_id', '$block_type', '$content', '$image_url', '$caption', '$sort_order')";
                $mysqli->query($sql);
            }
        ...
        header("Location: ../index.php?action=quanlychitietbaiviet&success=1");
        exit();
    } else {
...
if (isset($_GET['id'])) {
    ...
    $sql_info = "SELECT hinhanh FROM danhmucc WHERE iddanhmuc = '$id'";
    $result = $mysqli->query($sql_info);
    $article = $result->fetch_assoc();
    
    // Lấy thông tin các block để xóa ảnh
    $sql_blocks = "SELECT image_url FROM chitietdanhmuc WHERE iddanhmuc = '$id' AND block_type = 'image'";
    $blocks_result = $mysqli->query($sql_blocks);
    
    // Xóa các block thuộc bài viết trước
    $sql_delete_blocks = "DELETE FROM chitietdanhmuc WHERE iddanhmuc = '$id'";
    if ($mysqli->query($sql_delete_blocks)) {
        // Sau đó xóa bài viết
        $sql_delete_article = "DELETE FROM danhmucc WHERE iddanhmuc = '$id'";
        if ($mysqli->query($sql_delete_article)) {
            ...
            header("Location: ../index.php?action=quanlychitietbaiviet&success=3");
            exit();
        } else {
...
if (isset($_POST['suaCTbaiviet'])) {
    ...
    $sql = "UPDATE danhmucc SET title = '$title', hinhanh = '$hinhanh', mota = '$mota' WHERE iddanhmuc = '$iddanhmuc'";
    
    if ($mysqli->query($sql)) {
        // 2. Xóa các block nội dung cũ
        $sql_delete = "DELETE FROM chitietdanhmuc WHERE iddanhmuc = '$iddanhmuc'";
        ...
        header("Location: ../index.php?action=quanlychitietbaiviet&success=2");
        exit();
    } else {
